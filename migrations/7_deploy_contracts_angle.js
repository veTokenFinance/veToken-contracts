...
module.exports = async function (deployer, network, accounts) {
  ...
  let exchangeAddress = await sushiV2Factory.getPair(angle.address, ve3Token.address);

  if(exchangeAddress === constants.ZERO_ADDRESS){
    let newExchangeResult = await sushiV2Factory.createPair(angle.address, ve3Token.address);
