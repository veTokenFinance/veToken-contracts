...
contract("veToken Staking Reward Test", async (accounts) => {
  ...
  it("Deposit lpToken, get Rewards", async () => {
    ...
    console.log(
      "userA veToken before getReward():" +
        (await vetoken.balanceOf(userA)).toString()
    );
    await rewardPool.getReward();
    // 1.83% from  LP pools profits veAsset tokens
    console.log(
      "userA veAssertToken after getReward():" +
        (await veassetToken.balanceOf(userA)).toString()
    );
    ...
    console.log(
      "rewardPool veAssertToken after getReward():" +
        (await veassetToken.balanceOf(rewardPool.address)).toString()
    );
    ...
    console.log(
      "userA veAssetToken after withdraw:" +
        userAveAssetTokenAfterWithdraw1.toString()
    );
    const userAveTokenAfterWithdraw1 = await vetoken.balanceOf(userA);
    console.log(
      "userA veToken after withdraw:" + userAveTokenAfterWithdraw1.toString()
    );
    ...
    console.log(
      "userA veAssetToken after withdraw All:" +
        userAveAssetTokenAfterWithdraw2.toString()
    );
    expect(
      Number(
        (
          userAveAssetTokenAfterWithdraw2 - userAveAssetTokenAfterWithdraw1
        ).toString()
      )
    ).to.greaterThan(0);
    ...
    expect(
      Number(
        (userAveTokenAfterWithdraw2 - userAveTokenAfterWithdraw1).toString()
      )
    ).to.greaterThan(0);
    console.log(
      "userA veToken after withdraw All:" +
        userAveTokenAfterWithdraw2.toString()
    );
    ...
    await time.advanceBlock();
    await rewardPool.getReward();
    const userAveAssetTokenAfterWithdrawAll = await veassetToken.balanceOf(
      userA
    );
    expect(
      Number(
        (
          userAveAssetTokenAfterWithdrawAll - userAveAssetTokenAfterWithdraw2
        ).toString()
      )
    ).to.equal(0);
    ...
    expect(
      Number(
        (userAveTokenAfterWithdrawAll - userAveTokenAfterWithdraw2).toString()
      )
    ).to.equal(0);

    //withdraw again when userA has no lptoken staked, should revert
    await expectRevert(
      rewardPool.withdrawAndUnwrap(wei("10"), true, { from: userA }),
      "revert"
    );
